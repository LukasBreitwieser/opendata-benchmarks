...
//namespace ROOT {
  ...
  //namespace Math {
     ...
     */
        ...
        __device__ inline Scalar Eta_FromRhoZ(Scalar rho, Scalar z) {
           if (rho > 0) {

              // FIXME removed static qualifier from big_z_scaled
              // value to control Taylor expansion of sqrt
              const Scalar big_z_scaled = pow(::cuda::std::numeric_limits<Scalar>::epsilon(), static_cast<Scalar>(-.25));
