...
// Last update: $Id$
//
#ifndef DEVICEPTETAPHIMVECTOR_H_
#define DEVICEPTETAPHIMVECTOR_H_
...
public :
   ...
   */
   ...
   /**
      assignment operator
    */
   DevicePtEtaPhiM4D & operator = (const DevicePtEtaPhiM4D & v) {
      fPt  = v.fPt;
      fEta = v.fEta;
      fPhi = v.fPhi;
      fM   = v.fM;
   ...
   */
   Scalar Mt() const {
      ...
      if (mm >= 0) {
         ...
         throw std::runtime_error  ("PtEtaPhiM4D::Mt() - Tachyonic:\n"
...
public:
   ...
   */
   void Negate( ) {
      ...
      fEta = - fEta;
      throw std::runtime_error("PtEtaPhiM4D::Negate - cannot negate the energy - can negate only the spatial components");
   }
   ...
   */
   ...
   /**
      Exact equality
   */
   bool operator == (const DevicePtEtaPhiM4D & rhs) const {
      return fPt == rhs.fPt && fEta == rhs.fEta
         && fPhi == rhs.fPhi && fM == rhs.fM;
   }
   bool operator != (const DevicePtEtaPhiM4D & rhs) const {return !(operator==(rhs));}
